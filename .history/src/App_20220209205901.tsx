  ...
  return (
    ...
    <div className="App">
      <header className="App-header">
        ...
        <div> <Cart childId={2} date={new Date(card3.date).toLocaleDateString()} cartItems={card3.products} child={0}  /></div>
        <div>
         <p>Cart 4 date:{new Date(card4.date).toLocaleDateString()}</p>
          </div> 
        <div> <Cart childId={3} date={new Date(card4.date).toLocaleDateString()} cartItems={card4.products} child={0}  /></div> 
        <div>
         <p>Cart 5 date:{new Date(card5.date).toLocaleDateString()}</p>
          </div>
        <div> <Cart childId={4} date={new Date(card5.date).toLocaleDateString()} cartItems={card5.products} child={0}  /></div> 
       <div>
